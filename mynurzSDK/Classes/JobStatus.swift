...
public class JobStatusController {
    ...
    public func get(byId id: Int) -> JobStatus? {
        self.realm = try! Realm()
        return self.realm!.objects(JobStatus.self).filter("id = '\(id)'").first
    }
    
    public func get(byName name: String) -> JobStatus? {
        self.realm = try! Realm()
        return self.realm!.objects(JobStatus.self).filter("name = '\(name)'").first
    }
...
public class JobStatusTablePicker: UITableViewController, UISearchResultsUpdating {
    
    var searchController: UISearchController?
    open var customFont: UIFont?
    open var didSelectClosure: ((Int, String) -> ())?
    var filteredData = [JobStatus]()
    var data = [JobStatus]()
    let controller = JobStatusController.sharedInstance
    ...
    func filter(_ searchText: String){
        filteredData = [JobStatus]()
        
        self.data.forEach({ jobStatus in
            if jobStatus.name.characters.count > searchText.characters.count {
                let result = jobStatus.name.compare(searchText, options: [.caseInsensitive, .diacriticInsensitive], range: searchText.characters.startIndex ..< searchText.characters.endIndex)
                if result == .orderedSame {
                    filteredData.append(jobStatus)
                }
            }
        })
