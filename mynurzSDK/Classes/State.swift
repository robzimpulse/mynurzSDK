...
public class StateController {
    ...
    public func get(byId id: Int) -> State? {
        self.realm = try! Realm()
        return self.realm!.objects(State.self).filter("id = '\(id)'").first
    }
    
    public func get(byName name: String) -> State? {
        self.realm = try! Realm()
        return self.realm!.objects(State.self).filter("name = '\(name)'").first
    }
...
public class StateTablePicker: UITableViewController, UISearchResultsUpdating {
    
    var searchController: UISearchController?
    open var customFont: UIFont?
    open var didSelectClosure: ((Int, String) -> ())?
    var filteredData = [State]()
    var data = [State]()
    let controller = StateController.sharedInstance
    ...
    func filter(_ searchText: String){
        ...
        self.data.forEach({ state in
            if state.name.characters.count > searchText.characters.count {
                let result = state.name.compare(searchText, options: [.caseInsensitive, .diacriticInsensitive], range: searchText.characters.startIndex ..< searchText.characters.endIndex)
