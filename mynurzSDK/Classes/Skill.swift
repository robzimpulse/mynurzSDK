...
public class SkillTablePicker: UITableViewController, UISearchResultsUpdating {
    ...
    func filter(_ searchText: String){
        ...
        self.data.forEach({ skill in
            if skill.name.characters.count > searchText.characters.count {
                let result = skill.name.compare(searchText, options: [.caseInsensitive, .diacriticInsensitive], range: searchText.characters.startIndex ..< searchText.characters.endIndex)
                if result == .orderedSame {
                    filteredData.append(skill)
                }
            }
        })
