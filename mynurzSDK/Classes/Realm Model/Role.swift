...
public class RoleController {
    ...
    public func get(byId id: Int) -> Role? {
        self.realm = try! Realm()
        return self.realm!.objects(Role.self).filter("id = '\(id)'").first
    }
    
    public func get(byName name: String) -> Role? {
        self.realm = try! Realm()
        return self.realm!.objects(Role.self).filter("name = '\(name)'").first
    }
...
public class RoleTablePicker: UITableViewController, UISearchResultsUpdating {
    
    var searchController: UISearchController?
    open var customFont: UIFont?
    open var didSelectClosure: ((Int, String) -> ())?
    var filteredData = [Role]()
    var data = [Role]()
    let controller = RoleController.sharedInstance
    ...
    func filter(_ searchText: String){
        ...
        self.data.forEach({ role in
            if role.name.characters.count > searchText.characters.count {
                let result = role.name.compare(searchText, options: [.caseInsensitive, .diacriticInsensitive], range: searchText.characters.startIndex ..< searchText.characters.endIndex)
