...
public class DistrictController {
    ...
    public func get(byId id: Int) -> District? {
        self.realm = try! Realm()
        return self.realm!.objects(District.self).filter("id = '\(id)'").first
    }
    
    public func get(byName name: String) -> District? {
        self.realm = try! Realm()
        return self.realm!.objects(District.self).filter("name = '\(name)'").first
    }
...
public class DistrictTablePicker: UITableViewController, UISearchResultsUpdating {
    
    var searchController: UISearchController?
    open var customFont: UIFont?
    open var didSelectClosure: ((Int, String) -> ())?
    var filteredData = [District]()
    var data = [District]()
    let controller = DistrictController.sharedInstance
    ...
    func filter(_ searchText: String){
        ...
        self.data.forEach({ district in
            if district.name.characters.count > searchText.characters.count {
                let result = district.name.compare(searchText, options: [.caseInsensitive, .diacriticInsensitive], range: searchText.characters.startIndex ..< searchText.characters.endIndex)
                if result == .orderedSame {
                    filteredData.append(district)
                }
            }
        })
